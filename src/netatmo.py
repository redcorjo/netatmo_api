...
def main():
    ...
    if config.sections() == []:
        logger.warning(f"Created settings file {settings_file} with template data")
        config["credentials"] = {}
    ...
    netatmo = Netatmo_API(client_id, client_secret, username, password, home_id)
    if len(sys.argv) > 0:
        parameter = sys.argv[1]
    else:
        parameter = "schedule"
    response = netatmo.setthermmode(mode=parameter)
    return None
